...
<template>
  <div class="my-20 max-w-lg mx-auto bg-white shadow p-10">
    ...
    <div class="my-5">
      ...
      <p class="text-center my-10 text-4xl font-extrabold text-indigo-600">{{formatearDinero(cantidad)}}</p>
    ...
    <div v-if="total > 0" class="my-5 space-y-3 bg-gray-100 p-5">
      ...
      <p class="text-xl text-gray-500 text-center font-bold">
        Total a pagar: <span class="text-indigo-600">{{formatearDinero(total)}}</span></p>
      <p class="text-xl text-gray-500 text-center font-bold">Pago mensual: 
        <span class="text-indigo-600">{{formatearDinero(pagoMensual)}}</span> </p>

    </div>

    <p v-else class="text-center font-bold text-gray-700">Agrega una cantidad y un plazo a pagar</p>
  </div>
</template>
